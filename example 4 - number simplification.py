...
# Algebraica automatically uses Euler`s formula for complex exponeentials
print('exp(i) =', cp( ((1, 1), ('exp', cp(0, 1))) ).simplified())

# if you don`t want that, switch ExpandComplexExponentials to False
Algebraica.ExpandComplexExponentials = False

# now the exponential function is preserved
print('exp(i) =', cp( ((1, 1), ('exp', cp(0, 1))) ).simplified())
print()


print('cos(1) =', cp( ((1, 1), ('cos', cp(1))) ).simplified())
print('ctg(1) =', cp( ((1, 1), ('ctg', cp(1))) ).simplified())
# if you don`t like cos and ctg being simplified to sin and tan,
# turn off the ReplaceTrig setting

Algebraica.ReplaceTrig = False

# now cos and ctg are left on their own
print('cos(1) =', cp( ((1, 1), ('cos', cp(1))) ).simplified())
print('ctg(1) =', cp( ((1, 1), ('ctg', cp(1))) ).simplified())
print()
